...
# Do only 1:24 so as not to add AMBA twice, silly
national_populations_t[nrow(national_populations_t)+1,] <- c("National",sum(national_populations_t$TotalPopulation)-as.numeric(national_populations_t$TotalPopulation[25]),
                        sum(national_populations_t$MalePopulation)-as.numeric(national_populations_t$MalePopulation[25]),
                        sum(national_populations_t$FemalePopulation)-as.numeric(national_populations_t$FemalePopulation[25]))
...
icu_beds <- icu_frame %>% filter(variable=="beds")
...
beds_plot <- ggplot(icu_frame,aes(x = date, na.rm=TRUE)) +   
    geom_col(data=icu_beds,mapping=aes(x=date,y=value,na.rm=TRUE,color="gray"),size=.5) + 
    scale_x_continuous(name="Date",breaks=c(seq(first_icu_month,last_icu_month, by="month"))) +
    scale_y_continuous(name="ICU occupancy percentage") +
    theme(axis.text.x = element_text(angle=45,vjust=1,hjust=1),legend.position = "right") +
    scale_color_manual(name="Series",labels=c("Occupied ICU Beds"),values=c("gray")) +
    guides(fill = "legend") +
    labs(title="ICU Bed Occupancy, Nationwide")
